...
class DistributedRandomCategorySampler(Sampler):
    ...
    def __iter__(self):
        ...
        while len(avai_labels) >= self.num_labels_per_batch:
            random.seed(self.manual_seed + self.epoch)
            selected_labels = random.sample(avai_labels,
        ...
        local_indices = final_idxs[local_num * self.rank:local_num *
                                   (self.rank + 1)]
    ...
    def __iter__(self):
        ...
        for label in self.unique_labels:
            ...
            if remaining_idxs_num != 0:
                ...
                np.random.seed(self.manual_seed + self.epoch)
                need_idxs = np.random.choice(idxs,
                                             size=need_idxs_num,
                                             replace=True)
        ...
        local_groups = group_list[local_group_num * self.rank:local_group_num *
                                  (self.rank + 1)]
        local_indices = []
