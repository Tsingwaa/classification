...
class DistributedRandomCategorySampler(Sampler):
    ...
    def __iter__(self):
        ...
        local_indices = final_idxs[local_num * self.rank : local_num * (self.rank + 1)]
        print(f"Generated {local_num} index at rank {self.rank} by DistributedRandomCategorySampler.")
...
class DistributedRandomGroupSampler(Sampler):
    """
    Seperate data of each category into integral groups, and random sample groups to create
    batches for each iterations.
    Args:
    - labels (list): dataset labels.
    - num_instances (int): number of instances per category in a batch.
    ...
    def __init__(self, labels, batch_size, num_instances=8,
        ...
        if rank is None:
            if not dist.is_available():
                raise RuntimeError("Requires distributed package to be available")
            rank = dist.get_rank()
            print(f"Initialing DistributedRandomGroupSampler at rank {rank}...")
        ...
        self.length = math.floor(self.total_groups * 1.0 / num_replicas) * num_instances
        self.num_replicas = num_replicas
        self.rank = rank
    ...
    def __iter__(self):
        ...
        for label in self.unique_labels:
            ...
            # if there is remaining idxs, sample the needed num from this category idxs.
            ...
            if remaining_idxs_num != 0:
                ...
                np.random.seed(self.manual_seed + self.epoch)
                need_idxs = np.random.choice(idxs, size=need_idxs_num, replace=True)
        ...
        local_groups = group_list[local_group_num * self.rank : local_group_num * (self.rank + 1)]
        local_indices = []
        for group in local_groups:
            local_indices.extend(group)
        print(f"Generated {len(local_indices)} index at rank {self.rank} by DistributedRandomGroupSampler.")
