...
class GradualWarmupScheduler(_LRScheduler):
    ...
    Args:
        ...
        multiplier:
            if multiplier > 1.0, target lr = base lr * multiplier
            if multiplier = 1.0, lr starts from 0 and ends up with the base_lr.
        warmup_epochs: target lr is reached at warmup_epochs, gradually
        after_scheduler: after target_epoch, use this scheduler
    """

    def __init__(self, optimizer, multiplier, warmup_epochs,
                 after_scheduler=None):
        self.multiplier = multiplier
        if self.multiplier < 1.:
            raise ValueError('multiplier should not be less than 1.')
    ...
    def get_lr(self):
        ...
        if self.multiplier == 1.0:
            return [base_lr * (float(self.last_epoch) / self.warmup_epochs)
