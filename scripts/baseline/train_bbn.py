...
class Trainer(BaseTrainer):
    ...
    def train_epoch(self, cur_epoch, trainloader, model, criterion, optimizer,
        ...
        for i, (batch_imgs, batch_targets, meta) in enumerate(trainloader):

            batch_imgs = batch_imgs.cuda(non_blocking=True)
            batch_targets = batch_targets.cuda(non_blocking=True)
            imgs_b = meta["sample_image"].cuda(non_blocking=True)
            targets_b = meta["sample_label"].cuda(non_blocking=True)
    ...
    def evaluate(self, cur_epoch, valloader, model, criterion, dataset,
        ...
        with torch.no_grad():
            for i, (batch_imgs, batch_targets, meta) in enumerate(valloader):
                batch_imgs = batch_imgs.cuda(non_blocking=True)
                batch_targets = batch_targets.cuda(non_blocking=True)
                # batch_probs = model(batch_imgs, out_type="fc")
                feature = model(batch_imgs, feature_flag=True)
                batch_probs = model(feature, classifier_flag=True)
