...
class Trainer(BaseTrainer):
    def __init__(self, local_rank=None, config=None):
        ...
        self.trainloader2_params = config["trainloader2"]
    ...
    def train(self):
        ...
        train_sampler2 = self.init_sampler(self.train_sampler_name,
                                          dataset=trainset,
                                          **self.trainloader2_params)
        self.trainloader2 = DataLoaderX(trainset,
                                       batch_size=self.train_batchsize,
                                       shuffle=(train_sampler2 is None),
                                       num_workers=self.train_workers,
                                       pin_memory=True,
                                       drop_last=True,
                                       sampler=train_sampler)
    ...
    def train_epoch(self, cur_epoch, trainloader, trainloader2, model, criterion, opt,
                    num_classes, **kwargs):
        model.train()
        if self.local_rank in [-1, 0]:
            train_pbar = tqdm(
        ...
        # for i, (batch_imgs, batch_labels) in enumerate(trainloader):
            opt.zero_grad()
            train_batch_o = data[0][0].cuda()
            labels_batch_o = data[0][1].cuda()
            train_batch_r = data[1][0].cuda()
            labels_batch_r = data[1][1].cuda()

            batch_imgs, lam = self.mixup_data(train_batch_o, train_batch_r)
            # batch_imgs = batch_imgs.cuda()
            # batch_labels = batch_labels.cuda()
            batch_probs = model(batch_imgs, out_type='fc')
            avg_loss = self.mixup_criterion(criterion, batch_probs, labels_batch_o, labels_batch_r, lam)
            ...
            train_stat.update(labels_batch_o, batch_preds)
