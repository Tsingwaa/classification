...
@Losses.register_module("TripletCenterLoss")
class TripletCenterLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        targets_expand = targets.view(batch_size,
                                      1).expand(batch_size, inputs.size(1))
        ...
        for i in range(batch_size):  # for each sample, we compute distance
            dist_ap.append(dist[i][
                mask[i]].max())  # mask[i]: positive samples of sample i
            dist_an.append(dist[i][mask[i] == 0].min())
            # mask[i]==0: negative samples of sample i
        ...
        prec = (dist_an.data > dist_ap.data).sum() * 1. / y.size(
            0)  # normalize data by batch size
        return loss, prec
